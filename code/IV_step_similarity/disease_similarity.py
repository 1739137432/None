...
def diease_similarity():
    OriginalData = []
    ReadMyCsv(OriginalData, "../output/relationship/IV_step_similarity/dis2mi.csv")
    ReadMyCsv(OriginalData, "../output/relationship/IV_step_similarity/dis2circ.csv")
    ReadMyCsv(OriginalData, "../output/relationship/IV_step_similarity/dis2lnc.csv")
    ReadMyCsv(OriginalData, "../output/relationship/IV_step_similarity/dis2gene.csv")
    print(len(OriginalData))

    mesh = np.loadtxt("../input/Annotation/mesh_20230723/mesh_id.txt", dtype=str, delimiter=";")  # 读取源文件
    # mesh = pd.read_csv("mesh_id.csv", dtype=str, delimiter=";").values.tolist()
    print('mesh长度', mesh[0][1])
    ...
    with open('../output/relationship/IV_step_similarity/disease_adj_name.csv', 'w') as f:
        index = 0
        for row in AllDisease:
            f.write(str(index)+":" + str(row)+'\n')
            index+=1

    with open('../output/relationship/IV_step_similarity/disease_adj.csv', 'w') as f:
        for row in range(len(AllDisease)):
            for value in range(len(AllDisease)):
    ...
    while counter1 < len(OriginalData):
        ...
        while counter2 < len(AllRNA):
            if OriginalData[counter1][0] != AllRNA[counter2]:
                counter2 = counter2 + 1
            elif OriginalData[counter1][0] == AllRNA[counter2]:
                flag = 1
                break
        if flag == 0:
    ...
    with open('../output/relationship/IV_step_similarity/disease_similarity.csv', 'w') as f:
        for row in DiseaseSimilarity:
            for value in row:
                f.write(str(value)+'\n')