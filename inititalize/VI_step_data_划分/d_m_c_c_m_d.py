...
def d_m_c_c_m_d():
    ...
    for c1, c2 in c_c:
        if len(circRNA_miRNA_list[c1 - num_miRNA]) == 0 or len(circRNA_miRNA_list[c2 - num_miRNA]) == 0:
            continue
        candidate_m1_list = np.random.choice(len(circRNA_miRNA_list[c1 - num_miRNA]),int(0.1 * len(circRNA_miRNA_list[c1 - num_miRNA])),replace=False)
        candidate_m1_list = circRNA_miRNA_list[c1 - num_miRNA][candidate_m1_list]
        candidate_m2_list = np.random.choice(len(circRNA_miRNA_list[c2 - num_miRNA]),int(0.1 * len(circRNA_miRNA_list[c2 - num_miRNA])),replace=False)
        candidate_m2_list = circRNA_miRNA_list[c2 - num_miRNA][candidate_m2_list]
        m_c_c_m.extend([(m1, c1, c2, m2) for m1 in candidate_m1_list for m2 in candidate_m2_list])
    ...
    # g_m_c_c_m_g = np.array(g_m_c_c_m_g)
