...
def train_doc2vec_model(mers, name):
    ...
    model = Doc2Vec(vector_size=100, min_count=1, epochs=100)#实例化Doc2Vec 向量大小为100，迭代100次
...
def create_tagged_documents(mers, name):
    ...
    tagged_docs = [TaggedDocument(mers[i], name[i])#带标签的kmers
...
def miRNA_similarity():
    ...
    all_mers = mi_seq_mers  # 所有的k-mers list[['AGC',GCG...],[],...804]
    all_name = unique_mi  # #所有的RNAname list['','',...804]
    ...
    vectors = get_vector_embeddings(all_mers, all_name, pretrain_model)  # 生成维度为100的804个带标签的向量

    graph_embedding = np.zeros((len(all_name), 100))  # 生成零矩阵
    for node, vec in vectors.items():  # 将vertors转为矩阵
...
def lncRNA_similarity():
    ...
    for i in lnc_seq:
        ...
        # [[],[]..284]  [[],[]..520]
    all_mers = lnc_seq_mers  # 所有的k-mers list[['AGC',GCG...],[],...804]
    all_name = unique_lnc  # #所有的RNAname list['','',...804]
...
def gene_similarity():
    ...
    for i in gene_seq:
        ...
        # [[],[]..284]  [[],[]..520]
    all_mers = gene_seq_mers  # 所有的k-mers list[['AGC',GCG...],[],...804]
    all_name = unique_gene  # #所有的RNAname list['','',...804]
...
def circRNA_similarity():
    ...
    print(1)
    all_mers = circ_seq_mers  # 所有的k-mers list[['AGC',GCG...],[],...804]
    all_name = unique_circ  # #所有的RNAname list['','',...804]
